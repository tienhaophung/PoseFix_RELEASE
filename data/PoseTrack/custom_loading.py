...
# sys.path.insert(0, osp.join(osp.split(cur_dir)[0], "cocoapi", 'PythonAPI'))
...
class Dataset(object):
    ...
    input_pose_path = osp.join('..', 'data', dataset_name, 'input_pose', 'results.pkl') # set directory of the input pose
    ...
    def input_pose_load(self, image_path): # annot, db_set):
        
        # gt_img_id = self.load_imgid(annot)

        with open(self.input_pose_path, 'rb') as f:
            keypoints_list = pickle.load(f)
        
        input_pose = []
        for i in range(len(input_pose)):
            tmp = dict(image_id=keypoints_list[i]['track_id'], category_id=1)
            tmp['score'] = np.mean(keypoints_list[i]['keypoints'][[j for j in range(2, 15*3, 3)]] - 0.5)  
            # input_pose[i]['score'] = np.mean(input_pose[i]['scores'])
            input_pose.append(tmp)
        ...
        # bbox generate
        for i in range(len(input_pose)):
            input_pose[i]['estimated_joints'] = keypoints_list[i]['keypoints']
            ...
            input_pose[i]['bbox'] = keypoints_list[i]['bbox']
